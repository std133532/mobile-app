...
function signOutUserMenu() {
    alert("signOut");
      ...
      //Delete saved destinations for user 
    DeleteDestinations();
...
function DeleteDestinations() {
    ...
    firebase.firestore().collection("destinations").doc(getUserUid()).delete().then(function() {
...
function loadDestinations() {
    ...
    firebase.firestore().collection("destinations").where("uid", "==", getUserUid())
        .get()
        .then(function(querySnapshot) {
            querySnapshot.forEach(function(doc) {
                // doc.data() is never undefined for query doc snapshots
                ...
                var dest = doc.data();
                var points = dest.point;

                for (var i = 0; i < points.length; i++) {
                    var li = $('<li class="mdl-list__item mdl-list__item--three-line">');


                    var span_prim_cont = $('<span class="mdl-list__item-primary-content">  <i class="material-icons  mdl-list__item-avatar">person</i> ' + points[i].name.gr + '</span>')

                    span_prim_cont.appendTo(li);

                    var span_sec_cont = $('<span class="mdl-list__item-secondary-content destkm"><span class="mdl-list__item-secondary-info">' + ConvertDistance(points[i].distanceFromStation) + '</span> ');
                    span_sec_cont.appendTo(li);

                    var span_sec_action = $('<span class="mdl-list__item-secondary-action">  <label class="mdl-switch mdl-js-switch mdl-js-ripple-effect"><input type="checkbox" id="dest-" class="mdl-switch__input"/></label></span>');


                    span_sec_action.appendTo(li);


                    li.appendTo('#destinations-list');
                    $("#dest-").attr("id", "dest-" + i);
