...
//Returns the scooter with available Scooters
   ...
   //        alert("here1");
       ...
       /*  firebase.firestore().collection("escooters").where("status", "==", "vacant")
    .get()
    .then(function(querySnapshot) {
        querySnapshot.forEach(function(doc) {
            // doc.data() is never undefined for query doc snapshots
            console.log("getStationsWithScooters"+doc.id, " => ", doc.data());
            stationsIds.push(doc.data().stationID);
            alert(doc.data().stationID);
   ...
   */
   ...
   //  for (let i = 0; i < stationsIds.length; i++) {
                 firebase.firestore().collection("stations").where("stationID", "==","1").get()
        .then(function(querySnapshot) {
            querySnapshot.forEach(function(doc) {
                // doc.data() is never undefined for query doc snapshots
                alert('stationid'+doc.data().stationID);
                stationsList.push(doc.data());
                alert(stationsList)
